...
subtitle: "**Unidad 1: Repaso Modelo de Regresión Lineal**"
...
# El modelo de regresión lineal {.center .center-x}
...
4. Esperanza condicional cero para el término de error: 
$$\mathbb{E}[u_{i}|x]=0,\,\,\,\,\,x=x_{1i},x_{2i},...,x_{Ki}$$

5. Errores no autocorrelacionados y homocedásticos: 
$$\mathbb{V}(u_{i}|x)=\sigma^{2}\quad\forall i\quad y\quad \mathbb{Cov}(u_{i},u_{j}|x)=0\quad\forall i\neq j$$	

## Efectos marginales

Diferenciando Totalmente:
$$\Delta y=\beta_{1}\Delta x_{1i}+\beta_{2}\Delta x_{2i}+...+\beta_{K}\Delta x_{iK}+\Delta u_{i}$$
...
# Mínimos Cuadrados Ordinarios (MCO) {.center .center-x}
...
**Interpretación**: Efecto marginales estimados a partir de la muestra:
...
```{r ejemplo_1a}
db <- read_dta("http://fmwww.bc.edu/ec-p/data/wooldridge/ceosal1.dta")

lm(salary~roe, data = db) %>% 
    broom::tidy() %>% 
    knitr::kable(digits = 3, 
                 col.names = c("Param.", "Estimado", "Err. Est", "t", "Valor prob."),
                 align = "lcccc",
                 caption = "*Variable dependiente: salary*",)
```

## Ejemplo 1: Salarios de gerentes y resultados de la firma

```{r ejemplo_1b}
ggplot(db, aes(x=roe, y=salary)) + 
...
1.  La suma de los residuos de MCO es cero: $$\sum_{i=0}^{n}e_{i}=0$$

2.  Ortogonalidad: La covarianza muestral entre $x$ y $e$ es cero: $$\sum_{i=1}^{n}x_{ki}e_{i}=0\,\,\,\,k=1,...,K$$

3. El punto $(\bar{x},\bar{y})$ se encuentra siempre sobre la línea de regresión: $$\bar{y}=\hat{\beta}_{0}+\hat{\beta}_{1}\bar{x}_{1}+...+\hat{\beta}_{K}\bar{x}_{K}$$

## Implicaciones algebraicas de MCO

4. La media de los valores ajustados $\widehat{y}$ es igual a la media de los valores observados $y$: $$\bar{y}=\overline{\widehat{y}}$$
...
```{r ejemplo_2a}
db <- read_dta("http://fmwww.bc.edu/ec-p/data/wooldridge/401k.dta")

lm(formula = prate ~ mrate, data = db) %>% 
    broom::tidy() %>% 
    knitr::kable(digits = 3, 
                 col.names = c("Param.", "Estimado", "Err. Est", "t", "Valor prob."),
                 align = "lcccc",
                 caption = "*Variable dependiente: prate*")
```
    
    
   
```{r ejemplo_2b}
lm(prate~mrate+age, data = db)%>% 
    broom::tidy() %>% 
    knitr::kable(digits = 3, 
                 col.names = c("Param.", "Estimado", "Err. Est", "t", "Valor prob."),
                 align = "lcccc",
...
```{r ejemplo_2c}
lm(mrate~age, data = db) %>% 
    broom::tidy() %>% 
    knitr::kable(digits = 3, 
                 col.names = c("Param.", "Estimado", "Err. Est", "t", "Valor prob."),
                 align = "lcccc",
                 caption = "*Variable dependiente: mrate*")
```    


```{r}

reg_aux <- lm(mrate~age, data = db)
db$err_mrate <- reg_aux$residuals

lm(prate~err_mrate, data = db)%>% 
    broom::tidy() %>% 
    knitr::kable(digits = 3, 
                 col.names = c("Param.", "Estimado", "Err. Est", "t", "Valor prob."),
                 align = "lcccc",
...
# Bondad de ajuste {.center .center-x}

## Bondad de ajuste

Recordemos que:
$$y_i = \widehat{y}_{i} + e_i$$

Note que $STC=SEC+SRC$:
$$
\sum_{i=1}^{n}(y_{i}-\overline{y})^{2}=\sum_{i=1}^{n}(\widehat{y}_{i}-\overline{y})^{2}+\sum_{i=1}^{n}e_{i}^{2}
$$

La bondad de ajuste se puede medir entonces como:
$$
R^{2}=\frac{\sum_{i=1}^{n}(\widehat{y}_{i}-\overline{y})^{2}}{\sum_{i=1}^{n}(y_{i}-\overline{y})^{2}}=1-\frac{\sum_{i=1}^{n}e_{i}^{2}}{\sum_{i=1}^{n}(y_{i}-\overline{y})^{2}}
$$
...
-  $0\leq R^{2}\leq1$.
-  $R^{2}$ es no decreciente en el número de variables explicativas
incluidas en el modelo.
-  $\widehat{\beta}$ maximiza $R^{2}$.
-  $R^{2}$ es igual al cuadrado del coeficiente de correlación muestral
entre $y$ y $\hat{y}$.
...
Insesgamiento
: *Bajo los supuestos (1) a (4):
$$\mathbb{E}\left[\widehat{\beta}_{k}|x_{1i},...,x_{Ki}\right]=\beta_{k}\Rightarrow \mathbb{E}\left[\widehat{\beta}_{k}\right]=\beta_{k}\,\,\,\,k=0,1,...,K$$
para cualquier valor del parámetro poblacional $k$. En otras palabras, los estimadores de MCO son estimadores insesgados de los parámetros poblacionales.*

## Propiedades en muestras pequeñas

**Insesgamiento**: Considere los estimadores $\beta^*$ y $\hat{\beta}$:

![](figs/insesgamiento.png){fig-align="center"}

## Propiedades en muestras pequeñas

Bajo los supuesto (1) a (5):
$$
\mathbb{V}(\hat{\beta}_{k}|x)=\frac{\sigma^{2}}{\sum_{i=1}^{n}(x_{ki}-\bar{x}_{k})^{2}(1-R_{k}^{2})}
$$
donde $R_{k}^{2}$ es el $R^{2}$ de la regresión de $x_{k}$ sobre
todas las otras variables independientes (e incluyendo un intercepto).\bigskip{}


-  Más [ruido]{.emph} en la ecuación (una $\sigma^{2}$ más grande) dificulta
más estimar $\hat{\beta}_{k}$ de forma precisa.
-  Menor [variación muestral]{.emph} en $x_{k}$ hace más difícil estimar $\hat{\beta}_{k}$
de forma precisa.
-  Si $x_{k}$ y las otras variables explicativas están fuertemente correlacionadas ([colinealidad]{.emph}),
$R_{k}^{2}\rightarrow1$, se hace más difícil estimar $\hat{\beta}_{k}$
de forma precisa.
...
Tomemos el siguiente estimador:
$$
\tilde{\beta}_{k}=\sum_{i=1}^{n}w_{ki}y_{i}
$$
donde cada $w_{ki}$ es una función de los $x_{1i}$, $x_{2i}$, ...,$x_{K1}$. Se cumple que:
$$
\mathbb{V}(\hat{\beta}_{k}|x)\leq \mathbb{V}(\tilde{\beta}_{k}|x)
$$
...
![](figs/eficiencia.png){fig-align="center"}

## Propiedades en muestras pequeñas

**Supuesto adicional**:
	
6. El error poblacional $u_{i}$ es independiente de las variables explicativas y está distribuido normalmente, con media cero y varianza $\sigma^{2}$: $u\sim N(0,\sigma^{2})$.

![](figs/distribucionu.png){fig-align="center"}


## Propiedades en muestras pequeñas

Distribución Muestral de los Parámetros
: *Bajo los supuestos (1) a (6) del MLC: $\hat{\beta}_{k}|x\sim N\left(\beta_{k},\mathbb{V}(\hat{\beta}_{k}|x)\right)$ Por lo tanto: 
$$\frac{\hat{\beta}_{k}-\beta_{k}}{\sqrt{\mathbb{V}(\hat{\beta}_{k}|x)}}|x\sim N\left(0,1\right)$$*
...
- Entonces: 
$$\widehat{\mathbb{V}}(\hat{\beta}_{k}|x)=\frac{s^{2}}{\sum_{i=1}^{n}(x_{ki}-\bar{x}_{k})^{2}(1-R_{k}^{2})}$$


## Inferencia: Test t

Tenemos que:
$$\widehat{\beta}_{k}\sim N(\beta_{k},\mathbb{V}(\widehat{\beta}_{k}|x))$$
...
El primer paso es definir la hipótesis nula: 
$$H_{0}:\beta_{k}=\beta_{k}^{o}$$

## Inferencia: Test t

El estadístico $t$ bajo la hipótesis nula :
$$
t=\frac{\widehat{\beta}_{k}-\beta_{k}^{o}}{\sqrt{\widehat{\mathbb{V}}(\widehat{\beta}_{k}|x)}}
$$

Ademas es necesario definir una hipótesis alternativa, $H_{1}$, y
el nivel de significancia $\alpha$.


-  $H_{1}$ puede ser de una o dos colas.
-  $H_{1}:\beta_{k}>\beta_{k}^{o}$ y $H_{1}:\beta_{k}<\beta_{k}^{o}$
son de una cola.
...
![](figs/one_tail_t_test.png){fig-align="center"}

## Inferencia: Test t

Prueba de dos colas: $H_{0}:\beta_{k}=\beta_{k}^{o}$ y $H_{1}:\beta_{k}\neq\beta_{k}^{o}$

![](figs/two_tail_t_test.png){fig-align="center"}

## Inferencia: Estimación por intervalo

Usando el hecho que:
$$\frac{\widehat{\beta}_{k}-\beta_{k}}{\sqrt{\widehat{\mathbb{V}}(\widehat{\beta}_{k}|x)}}\sim t_{n-K-1}$$

tenemos que:
$$\Pr\left[-t_{\alpha/2,n-K-1}\leq\frac{\widehat{\beta}_{k}-\beta_{k}}{\sqrt{\widehat{\mathbb{V}}(\widehat{\beta}_{k}|x)}}\leq t_{\alpha/2,n-K-1}\right]=1-\alpha$$

## Inferencia: Estimación por intervalo

Mediante manipulaciones algebraicas obtenemos: 
$$\Pr\left[\widehat{\beta}_{k}-t_{\alpha/2,n-K-1}\sqrt{\widehat{\mathbb{V}}(\widehat{\beta}_{k}|x)}\leq\beta_{k}\leq\widehat{\beta}_{k}+t_{\alpha/2,n-K-1}\sqrt{\widehat{\mathbb{V}}(\widehat{\beta}_{k}|x)}\right]=1-\alpha$$

Estimación por intervalo:
$$\widehat{\beta}_{k}\pm t_{\alpha/2,n-K-1}\sqrt{\widehat{\mathbb{V}}(\widehat{\beta}_{k}|x)}$$
...
Note que:
$$\widehat{\mathbb{V}}(\widehat{\beta}_{1}-\widehat{\beta}_{2})=\widehat{\mathbb{V}}(\widehat{\beta}_{1})+\widehat{\mathbb{V}}(\widehat{\beta}_{2})-2\widehat{\mathbb{Cov}}(\widehat{\beta}_{1},\widehat{\beta}_{2})$$

## Inferencia: Test de exclusión de variables

Tenemos la siguiente hipótesis nula (significancia global):
...
El test se realiza comparando el modelo irrestricto con el modelo restringido. Se usa el estadístico $F$:
$$F=\frac{(SRC_{R}-SRC_{NR})/q}{SRC_{NR}/n-K-1}\sim F_{q,n-K-1}$$

:::

::: {.column width="50%"}

![](figs/f_test.png){fig-align="center"}
:::
::::
...
```{r}
db <- read_dta("http://fmwww.bc.edu/ec-p/data/wooldridge/gpa1.dta")

lm(colGPA~hsGPA+ACT+skipped, data = db) %>% 
    broom::tidy() %>% 
    knitr::kable(digits = 3, 
                 col.names = c("Param.", "Estimado", "Err. Est", "t", "Valor prob."),
                 align = "lcccc",
                 caption = "*Variable dependiente: colGPA*")
...
```{r}
reg1 <- lm(colGPA~hsGPA+ACT+skipped, data = db) 
linearHypothesis(reg1, c("hsGPA = 0", "ACT = 0", "skipped = 0"))
```
...
```{r}
linearHypothesis(reg1, "hsGPA - ACT = 0")
```
    
# Variables dummy {.center .center-x}
...
-  Considere el modelo de regresión poblacional:
$$y_i=\beta_0+\delta_0 d_i + u_i$$
con $d_i$ una variable dummy. Por ejemplo: $y=salario$ y $d=mujer$.
-  Suponemos que se cumple el supuesto $E[u_i|d_i]=0$ y por tanto:
$$E[y_i|d_i]=\beta_0+\delta_0 d_i$$
-  ¿Qué interpretación tiene $\delta_0$? 

## Variables dummy en el contexto de regresión simple

- Como $d_i$ toma sólo dos valores tenemos:
\begin{eqnarray*}
E[y_i|d_i=1]&=&\beta_0+\delta_0 \\
E[y_i|d_i=0]&=&\beta_0
\end{eqnarray*}
por tanto:
$$\delta_0 = E[y_i|d_i=1]- E[y_i|d_i=0]$$
...
```{r}
db <- read_dta("http://fmwww.bc.edu/ec-p/data/wooldridge/wage1.dta")
head(db[,1:7], n=10)
```
...
```{r}
aggregate(wage~female, data = db, FUN = mean)
```

- Efectivamente $7.09-4.58 = -2.51$.

## Variables dummy en el contexto de regresión múltiple

Considere el modelo de regresión poblacional:
$$y_i=\beta_0+\beta_1 x_i + \delta_0 d_i + u_i$$
Por ejemplo: $y=salario$, $x=educ$ y $d=mujer$.

\ 

¿Qué interpretación tiene $\delta_0$ en este contexto?
\begin{eqnarray*}
    E[y_i|x_i,d_i=1]&=&\beta_0+\delta_0 + \beta_1 x_i \\
...
Al igual que antes: 
$$\delta_0 = E[y_i|x_i,d_i=1]- E[y_i|x_i,d_i=0]$$
para cualquier nivel de $x_i$. 
...
```{r}
reg2 <-  lm(wage~educ+female, data = db)
...
```{r}
#|fig-align: center
db_male = subset(db, female==0)
db_female = subset(db, female==1)
db_male$wage_hat = predict(reg2, newdata = db_male)
db_female$wage_hat = predict(reg2, newdata = db_female)
plot(x=db_male$educ, y=db_male$wage_hat, type = "l", lwd =2, col = "blue",
     ylab = "Wage", xlab = "Education", ylim = c(0,10))
lines(x=db_female$educ, y=db_female$wage_hat, lwd =2, col = "red")
```

## Variables dummy en el contexto de regresión múltiple

-  Considere el modelo de regresión poblacional con un término de interacción:
$$y_i=\beta_0+\beta_1 x_i + \delta_0 d_i + \delta_1 d_1 x_i + u_i$$
Por ejemplo: $y=salario$, $x=educ$ y $d=mujer$.

-  ¿Qué interpretación tienen $\delta_0$y $\delta_1$ en este contexto?
\begin{eqnarray*}
E[y_i|x_i,d_i=1]&=&(\beta_0+ \delta_0) + (\beta_1 + \delta_1)x_i \\
E[y_i|x_i,d_i=0]&=&\beta_0 + \beta_1 x_i
\end{eqnarray*}

## Variables dummy en el contexto de regresión múltiple
- Ahora tenemos:
$$E[y_i|x_i,d_i=1]- E[y_i|x_i,d_i=0]=\delta_0 + \delta_1 x_i$$ 
esto es la diferencia de promedios depende de $x_i$. 
...
```{r}
reg3 <- lm(wage~educ+female+educ*female, data = db)
...
```{r}
#|fig-align: center
db_male$wage_hat_int = predict(reg3, newdata = db_male)
db_female$wage_hat_int = predict(reg3, newdata = db_female)
plot(x=db_male$educ, y=db_male$wage_hat_int, type = "l", col = "blue", lwd = 2,
     ylab = "Wage", xlab = "Education", ylim = c(0,10))
lines(x=db_female$educ, y=db_female$wage_hat_int, col = "red", lwd = 2)
```

# Violaciones a los supuestos del modelo clásico de regresión {.center .center-x}
