...
$$H_0: pe_t = pe_{t-1} = pe_{t-2}=0$$ 
  es `r testF$F[2]` con un p-value de `r testF$'Pr(>F)'[2]`.

## Ejemplo 6: Exención tributaria y tasas de fertilidad

- Multiplicador de largo plazo, $MLP = pe_t +pe_{t-1} +pe_{t-2}$, es `r mult_lp`.

- Test F para la hipótesis $H_0: pe_t + pe_{t-1} + pe_{t-2}=0$ 
...
## Ejemplos de series de tiempo débilmente dependientes
 
- Proceso de [medias móviles de orden uno]{.emph}, MA(1).
